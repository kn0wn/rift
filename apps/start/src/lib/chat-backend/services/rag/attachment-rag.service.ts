...
export const AttachmentRagLive = Layer.succeed(AttachmentRagService, {
  indexAttachmentChunks: ({ chunks }) =>
    Effect.tryPromise({
      try: () => insertAttachmentVectors({ chunks }),
      catch: (error) => error,
    }),
  linkAttachmentToThread: (input) =>
    Effect.tryPromise({
      try: () => linkAttachmentVectorsToThread(input),
      catch: (error) => error,
    }),
  searchThreadAttachments: ({ request }) =>
    Effect.tryPromise({
      try: async () => {
        if (
